...
class UserDelegationFiltersService
{
    public function apply(): Builder
    {
        $query = UserDelegation::query()->with(['principal', 'substitute']);

        // -- Pobierz filtry z sesji lub request
        $filters = session()->get('filters.delegations', []);
    ...
    public function save(array $filters): void
    {
        ...
        match($submit) {
            'filter' => request()->session()->put('filters.delegations', $filters),
            default => request()->session()->forget('filters.delegations'),
        };
    }
}

...
@section('content')
    <x-page-title title="Department Details">
        <x-slot name="breadcrumbs">
            <x-breadcrumbs text="Department Details">
                <x-breadcrumbs.item href="{{ route('departments.index') }}" text="Departments" />
            </x-breadcrumbs>
        </x-slot>
    </x-page-title>
    ...
    <div class="grid gap-4 xl:grid-cols-2 2xl:grid-cols-3">
        ...
        <div class="flex items-center justify-between pt-3 mt-4 border-t border-gray-200 sm:pt-6 dark:border-gray-700">
          <div>
            ...
            <!-- Dropdown menu -->
            <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow dark:bg-gray-700 dark:divide-gray-600" id="weekly-sales-dropdown" data-popper-placement="top" style="position: absolute; inset: auto auto 0px 0px; margin: 0px; transform: translate(355px, 81px);" data-popper-reference-hidden="" data-popper-escaped="">
        ...
        <ul class="text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            <li class="w-full px-4 py-4 border-b bg-gray-100 border-gray-200 dark:border-gray-600">
                {{ $department -> name }} ({{ $department -> members -> count() }})
            </li>

            @forelse ($department->members as $member)
                <li class="w-full px-4 py-3 border-b border-gray-200 dark:border-gray-600">
                    {{ $member -> full_name }}
                </li>
            @empty
                <li class="w-full px-4 py-3 rounded-b-lg">Brak Pracowników</li>
            @endforelse
        </ul>
    </div>

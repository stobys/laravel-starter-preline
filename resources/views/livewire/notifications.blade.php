                ...
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="w-full md:w-1/3">
                        <div class="flex-row items-center justify-between sm:flex sm:space-y-0 sm:space-x-4">
                            <div>
                                <h5 class="mr-3 font-semibold dark:text-gray-300">{{ __('notifications.labels.notifications_list') }}</h5>
                                <p class="text-adient-teal-3 dark:text-gray-400 text-sm">{{ __('notifications.labels.notifications_list_helper') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="w-full md:w-1/3">
                        ...
                        <div class="flex items-center space-x-3 w-full md:w-auto">
                            <div class="flex w-40">
                                <label class="shrink-0 z-10 inline-flex items-center p-2 px-3 text-xs h-9 font-medium text-center text-gray-900 bg-gray-100 border border-gray-300 rounded-s-lg dark:bg-gray-700 dark:text-white dark:border-gray-600" type="button">
                                    @svg('feather-eye', 'w-4 h-4')
                                </label>
                                <select wire:model.live.debounce.300ms="is_read" class="w-40 p-2 px-3 text-xs h-9 text-gray-900 bg-gray-100 border border-gray-300 rounded-e-lg cursor-pointer w-36 sm:block hover:bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:placeholder:text-gray-400 dark:text-white">
                                    @foreach ($statuses as $code => $label)
                                        <option value="{{ $code }}" @selected($is_read == $code)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400 opacity-70">
                            <tr>
                                ...
                                --}}
                                <th wire:click="sortBy('message')" scope="col" class="px-4 py-3 cursor-pointer">
                                    <p class="antialiased flex items-center justify-between gap-2 leading-none">
                                        {{ __('notifications.th.message') }}
                                ...
                                <th wire:click="sortBy('date')" scope="col" class="px-4 py-3 cursor-pointer">
                                    <p class="antialiased flex items-center justify-between gap-2 leading-none">
                                        {{ __('notifications.th.date') }}
                                ...
                                <th wire:click="sortBy('read')" scope="col" class="px-4 py-3 cursor-pointer">
                                    <p class="antialiased flex items-center justify-between gap-2 leading-none">
                                        {{ __('notifications.th.read') }}
                        ...
                        <tbody>
                            @forelse($models as $model)
                                ...
                                >
                                    ...
                                    <td class="w-64 px-4 py-3 whitespace-nowrap">
                                        {{ $model->created_at?->format('Y-m-d H:i') }}
                                    </td>
                                    <td class="w-64 px-4 py-3 whitespace-nowrap">
                                        {{ $model->read_at?->format('Y-m-d H:i') }}
                                    </td>
                                    <td class="w-64 px-4 py-3 text-center justify-end content-center text-right">
                                        <a href="#toggle" wire:click="readToggle({{ $model->id }})"
                                        ...
                                        >
                                            @if($model->read_at)
                                                @svg('feather-eye-off', 'w-4 h-4 text-red-600')
                                            @else
                                                @svg('feather-eye', 'w-4 h-4 text-green-600')
                                            @endif
                                        </a>
                                        {{-- <x-tooltip id="tooltip-active-{{ $model->id }}" placement="bottom">
                                            {{ $model->read_at ? __('notifications.actions.mark_as_unread') : __('notifications.actions.mark_as_read') }}
                                        </x-tooltip> --}}

                                        <a href="{{ route('notifications.details', $model->id) }}"
                                        ...
                                        <a x-data href="{{ route('notifications.delete', $model->id) }}"
                                            @click.prevent="if (window.confirm('{{ __('notifications.actions.delete_prompt') }}')) { window.location = $el.href }"
                                            class="inline-flex items-center py-2 px-2 text-sm font-medium rounded-lg text-red-700 hover:bg-gray-200 dark:text-primary-500 dark:hover:bg-gray-800"
                                        >

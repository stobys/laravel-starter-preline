        ...
        <div class="mx-auto w-full shadow-sm rounded-lg mb-4 border border-gray-200 dark:border-gray-700">
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
                <div class="flex-row items-center justify-between p-4 sm:flex sm:space-y-0 sm:space-x-4 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <h5 class="mr-3 font-semibold dark:text-gray-300">{{ __('departments.labels.notifications_list') }}</h5>
                        <p class="text-adient-teal-3 dark:text-gray-400 text-sm">{{ __('departments.labels.notifications_list_helper') }}</p>
                    </div>
                    ...
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <div class="flex items-center space-x-3 w-full md:w-auto">
                            ...
                            <div class="flex w-40">
                                ...
                                <select wire:model.live.debounce.300ms="state" class="hidden w-40 p-2 px-3 text-xs h-9 text-gray-900 bg-gray-100 border border-gray-300 rounded-e-lg cursor-pointer w-36 sm:block hover:bg-gray-50 dark:bg-gray-700 dark:border-gray-600 dark:placeholder:text-gray-400 dark:text-white">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400 opacity-70">
                            <tr>
                                ...
                                <th wire:click="sortBy('abbr')" scope="col" class="px-4 py-3 cursor-pointer">
                                    <p class="antialiased flex items-center justify-between gap-2 leading-none">
                                        {{ __('departments.th.abbr') }}
                                ...
                                <th wire:click="sortBy('manager')" scope="col" class="px-4 py-3 cursor-pointer">
                                    <p class="antialiased flex items-center justify-between gap-2 leading-none">
                                        {{ __('departments.th.manager') }}
                                ...
                                <th wire:click="sortBy('members_count')" scope="col" class="px-4 py-3 cursor-pointer">
                                    <p class="antialiased flex items-center justify-between gap-2 leading-none">
                                        {{ __('departments.th.members_count') }}
                        ...
                        <tbody>
                            @forelse($models as $model)
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3">{{ $model->members_count }}</td>
                                    <td class="px-4 py-3 text-center justify-end content-center text-right">
                                        @can('destroy', $model)
                                        <a href="#" wire:click="destroy({{ $model->id }})" wire:confirm="{{ __('department.actions.destroy_prompt') }}"
                                            class="inline-flex items-center py-2 px-2 text-sm font-medium rounded-lg text-red-700 hover:bg-gray-200 dark:text-primary-500 dark:hover:bg-gray-800"
                                        >
                                            @svg('feather-trash-2', 'w-4 h-4')
                                            {{-- {{ __('table.actions.withdraw') }} --}}
                                        </a>
                                        @endcan

                                        <a href="#view" wire:click="openPreviewDrawer({{ $model->id }})" data-drawer-target="department-preview-drawer"
                                            class="inline-flex items-center py-2 px-2 text-sm font-medium rounded-lg text-red-700 hover:bg-gray-200 dark:text-primary-500 dark:hover:bg-gray-800"
                                        >
                                            @svg('feather-file-text', 'w-4 h-4 mr-2')
                                            {{ __('table.actions.view') }}
                                        </a>

                                        <a href="{{ route('departments.show', $model->id) }}"
                                            class="inline-flex items-center py-2 px-2 text-sm font-medium rounded-lg text-red-700 hover:bg-gray-200 dark:text-primary-500 dark:hover:bg-gray-800"
                                        >
                                            @svg('heroicon-s-magnifying-glass', 'w-4 h-4 mr-2')
                                            {{ __('table.actions.show') }}
                                        </a>

                                        @can('update', $model)
                                        <a href="{{ route('departments.edit', $model->id) }}"
                                            class="inline-flex items-center py-2 px-2 text-sm font-medium rounded-lg text-red-700 hover:bg-gray-200 dark:text-primary-500 dark:hover:bg-gray-800"
                                        >
                                            @svg('heroicon-s-pencil', 'w-4 h-4 mr-2')
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-12 text-center">
                                        <h3 class="mt-2 text-sm font-medium text-gray-900">Brak działów</h3>
                                        <p class="mt-1 text-sm text-gray-500">
                                            @if($search)
                                                Nie znaleziono działów dla frazy "{{ $search }}"
                                            @else
                                                Zacznij od dodania nowego działu.
                                            @endif
                                        </p>
                                    </td>
                                </tr>

...
>
    ...
    <div
        ...
        class="relative border-2 border-dashed rounded-xl px-8 py-8 text-center
    ...
    >
        ...
        >
            ...
            <div class="w-52 bg-gray-200 rounded-full h-1.5 overflow-hidden">
                <div class="bg-blue-500 h-1.5 rounded-full transition-all duration-300"
                     :style="`width: ${progress}%`"></div>
            </div>
            <p class="text-xs text-gray-500 mt-2" x-text="`Przesyłanie… ${progress}%`"></p>
        </div>

        {{-- Ikona + tekst --}}
        <div x-show="!uploading" class="flex items-center justify-center gap-4 pointer-events-none">
            ...
            <div class="text-left">
                ...
                <p class="text-xs text-gray-400 mt-0.5">Wiele plików jednocześnie · max 10 MB każdy</p>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50">
                ...
                <div class="flex items-center gap-2">
                    @if(count($toDelete) > 0)
                        <span class="text-xs text-red-500 font-medium">{{ count($toDelete) }} do usunięcia</span>
                    @endif
            ...
            @if(count($existingAttachments) === 0)
                <div class="flex flex-col items-center justify-center py-12 text-center px-4">
                    ...
                    <p class="text-xs text-gray-400">Brak załączników</p>
                </div>
            @else
                <ul class="divide-y divide-gray-50 max-h-80 overflow-y-auto">
                    ...
                    <li class="flex items-start gap-3 px-4 py-3 transition-colors duration-150
                        ...
                        {{-- Ikona --}}
                        @php $ext = strtolower(pathinfo($attachment['original_name'], PATHINFO_EXTENSION)); @endphp
                        <div class="flex-shrink-0 mt-0.5 w-8 h-8 rounded-lg flex items-center justify-center
                            {{ in_array($ext, ['jpg','jpeg','png','gif','webp','svg']) ? 'bg-purple-100' :
                               ($ext === 'pdf' ? 'bg-red-100' :
                               (in_array($ext, ['xls','xlsx','csv']) ? 'bg-green-100' :
                               (in_array($ext, ['doc','docx']) ? 'bg-blue-100' : 'bg-gray-100'))) }}">
                            <svg class="w-4 h-4
                                {{ in_array($ext, ['jpg','jpeg','png','gif','webp','svg']) ? 'text-purple-500' :
                                   ($ext === 'pdf' ? 'text-red-500' :
                                   (in_array($ext, ['xls','xlsx','csv']) ? 'text-green-600' :
                                   (in_array($ext, ['doc','docx']) ? 'text-blue-500' : 'text-gray-400'))) }}"
                        ...
                        <div class="flex-shrink-0 flex items-center gap-0.5 mt-0.5">
                            @if(!$markedDelete)
                                ...
                                <button type="button" wire:click="markDelete({{ $attachment['id'] }})"
                            ...
                            @else
                                <button type="button" wire:click="unmarkDelete({{ $attachment['id'] }})"
                                    class="px-2 py-1 text-xs rounded text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors font-medium">
                                    Cofnij
                                </button>
                            @endif
                        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white overflow-hidden">
            ...
            @if(count($newFiles) === 0)
                <div class="flex flex-col items-center justify-center py-12 text-center px-4">
                    ...
                    <p class="text-xs text-gray-400">Dodaj pliki powyżej</p>
                </div>
            @else
                <ul class="divide-y divide-gray-50 max-h-80 overflow-y-auto">
                    @foreach($newFiles as $i => $file)
                    <li class="flex items-start gap-3 px-4 py-3 hover:bg-blue-50/30 transition-colors">
                        <div class="flex-shrink-0 mt-0.5 w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center">
    ...
    <div class="flex items-center justify-between pt-3 border-t border-gray-100">
        <p class="text-xs text-gray-400 space-x-2">
            @if(count($toDelete) > 0)
                <span class="text-red-500 font-medium">{{ count($toDelete) }} plik(ów) do usunięcia</span>
            @endif
            @if(count($toDelete) > 0 && count($newFiles) > 0)
                <span>·</span>
            @endif
            @if(count($newFiles) > 0)
                <span class="text-blue-600 font-medium">{{ count($newFiles) }} nowy(ch) do dodania</span>
            @endif
        </p>
...
Alpine.data('attachmentsManager', () => ({
    ...
    init() {
        this.$wire.$on('upload:start',    ()  => { this.uploading = true;  this.progress = 0; });
        this.$wire.$on('upload:finish',   ()  => { this.uploading = false; this.progress = 100; });
        this.$wire.$on('upload:error',    ()  => { this.uploading = false; });
        this.$wire.$on('upload:progress', (e) => { this.progress = e[0]?.progress ?? 0; });
    },
    ...
    handleFiles(files) {
        ...
        $wire.uploadMultiple(
            ...
            Array.from(files),
            () => {},
            () => { $wire.set('uploadError', 'Błąd przesyłania. Sprawdź rozmiar pliku (max 10 MB).'); },
            (event) => { this.progress = event.detail?.progress ?? 0; }
        );
    }
}));
</script>
